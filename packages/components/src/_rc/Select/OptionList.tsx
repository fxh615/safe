...
const OptionList: React.ForwardRefRenderFunction<RefOptionListProps, OptionListProps> = (
  ...
  const memoFlattenOptions = useMemo(
    () => flattenOptions,
    [open, flattenOptions],
    (prev, next) => (next[0] && prev[1] !== next[1]) as boolean,
  );
  ...
  const setActive = (index: number, fromKeyboard = false) => {
    ...
    if (!flattenItem) {
      onActiveValue(null as unknown as RawValueType, -1, info);
      return;
    }
    onActiveValue(flattenItem.value as RawValueType, index, info);
  };
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (!multiple && open && rawValues.size === 1) {
        const value: RawValueType = Array.from(rawValues)[0] as RawValueType;
        const index = memoFlattenOptions.findIndex(({ data }: any) => data.value === value);
    ...
    if (open) {
      listRef.current?.scrollTo(null as unknown as number | ScrollConfig);
    }
      ...
      switch (which) {
        ...
        case KeyCode.ENTER: {
          ...
          if (item && !item.data.disabled) {
            ...
            onSelectValue(null as unknown as RawValueType);
          }
  ...
  const omitFieldNameList = fieldNames ? Object.keys(fieldNames).map((key: string) => (fieldNames as any)[key]) : [];
  ...
  const renderItem = (index: number) => {
    ...
    return item ? (
      <div
        aria-label={typeof mergedLabel === 'string' && !group ? mergedLabel as string : null as unknown as string}
        {...attrs}
        key={index}
    ...
    <>
      ...
      >
        {(item, itemIndex) => {
          ...
          // Option
          const selected = isSelected(value as RawValueType);
          ...
          return (
            <div
              ...
              onClick={() => {
                if (!disabled) {
                  onSelectValue(value as RawValueType);
                }
              }}
              style={style}
            >
