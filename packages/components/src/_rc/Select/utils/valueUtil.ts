...
function getKey(data: BaseOptionType, index: number) {
  const { key } = data;
  let value: RawValueType = data.value;
...
export function getSeparatedContent(text: string, tokens: string[]): string[] {
  ...
  function separate(str: string, [token, ...restTokens]: string[]): string[] {
    ...
    const list: string[] = str.split(token);
    match = match || list.length > 1;

    return list
      .reduce((prevList: string[], unitStr: string) => [...prevList, ...separate(unitStr, restTokens)], [])
      .filter((unit: string) => unit);
  }

  const list = separate(text, tokens);
  return match ? list : null as unknown as string[];
}
