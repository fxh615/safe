...
import type { CustomTagProps, DisplayValueType, RenderNode } from '../BaseSelect';
...
const Selector: React.ForwardRefRenderFunction<RefSelectorProps, SelectorProps> = (props, ref) => {
  ...
  let pastedTextRef = useRef<string | null>(null);
  ...
  const selectNode =
    mode === 'multiple' || mode === 'tags' || mode === 'checkbox' ? (
      <MultipleSelector {...props} {...sharedProps} />
    ) : (
      <SingleSelector {...props} {...sharedProps} />
    );

  const singeleStyle = mode === 'checkbox' ? {
    FlexWrap: 'nowrap',
    height: '32px',
    overflow: 'hidden',
    WordBreak: 'keep-all',
