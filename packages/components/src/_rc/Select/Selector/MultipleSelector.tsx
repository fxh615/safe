...
function itemKey(value: DisplayValueType) {
  return value.key ?? value.value as string;
}
...
const SelectSelector: React.FC<SelectorProps> = (props) => {
  ...
  function renderItem(valueItem: DisplayValueType) {
    ...
    return typeof tagRender === 'function'
      ? customizeRenderSelector(value as RawValueType, displayLabel, itemDisabled as boolean, closable, onClose)
      : defaultRenderSelector(label, displayLabel, itemDisabled as boolean, closable, onClose);
  }
  ...
  const inputNode = (
    ...
    >
      <Input
        ...
        prefixCls={prefixCls}
        id={id}
        inputElement={null as unknown as React.ReactElement}
  ...
  return (
    <>
      { mode ==='checkbox' ? values.map((item: DisplayValueType) => item.label).join(',') : selectionNode}
