...
export default (
  ...
  const filledLabeledValues = React.useMemo(() => {
    ...
    // Fill label by cache
    const patchedValues = labeledValues.map((item: LabelInValueType) => {
    ...
    patchedValues.forEach((item: LabelInValueType) => {
      valueCache.set(item.value, item);
        optionCache.set(item.value, valueOptions.get(item.value) || prevOptionCache.get(item.value) || null as unknown as DefaultOptionType);
    });
  ...
  const getOption = React.useCallback(
    ...
    [valueOptions],
  ) as (val: RawValueType) => DefaultOptionType;
