...
import type { RefTriggerProps } from './SelectTrigger.interface';
...
export function isMultiple(mode: Mode) {
  return mode === 'tags' || mode === 'multiple' || mode === 'checkbox';
}
...
const BaseSelect = React.forwardRef((props: BaseSelectProps, ref: React.Ref<BaseSelectRef>) => {
  ...
  // ============================== MISC ==============================
  const multiple = isMultiple(mode as Mode);
  ...
  React.useImperativeHandle(ref, () => ({
    focus: selectorRef.current?.focus as () => void,
    blur: selectorRef.current?.blur as () => void,
  ...
  // ============================== Open ==============================
  const [innerOpen, setInnerOpen] = useMergedState<boolean>(undefined as unknown as boolean, {
    defaultValue: defaultOpen,
    value: open,
  });

  let mergedOpen = innerOpen as unknown as boolean;
  ...
  const onInternalSearch = (searchText: string, fromTyping: boolean, isCompositing: boolean) => {
    ...
    const patchLabels: string[] | null = isCompositing
      ? null
      : getSeparatedContent(searchText, tokenSeparators as string[]);
  ...
  const onInternalMouseDown: React.MouseEventHandler<HTMLDivElement> = (event, ...restArgs) => {
    ...
    const { target } = event;
    const popupElement: HTMLDivElement = triggerRef.current?.getPopupElement() as HTMLDivElement;
  ...
  // ============================ Dropdown ============================
  const [containerWidth, setContainerWidth] = React.useState<number>(null as unknown as number);
  ...
  useLayoutEffect(() => {
    if (triggerOpen) {
      const newWidth = Math.ceil(containerRef.current?.offsetWidth as number);
  ...
  const selectorNode = (
    <SelectTrigger
      ...
      dropdownStyle={dropdownStyle as React.CSSProperties}
      dropdownClassName={dropdownClassName as string}
      direction={direction as string}
      dropdownMatchSelectWidth={dropdownMatchSelectWidth}
      dropdownRender={dropdownRender}
      dropdownAlign={dropdownAlign as AlignType}
      placement={placement}
  ...
  return (
    <BaseSelectContext.Provider value={baseSelectContext as any}>{renderNode}</BaseSelectContext.Provider>
  );
});
