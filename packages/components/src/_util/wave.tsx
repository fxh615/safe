...
class InternalWave extends React.Component<WaveProps> {
  ...
  private extraNode: HTMLDivElement | undefined;

  private clickWaveTimeoutId: number | undefined;

  private animationStartId: number | undefined;
  ...
  onClick = (node: HTMLElement, waveColor: string) => {
    ...
    ['transition', 'animation'].forEach(name => {
      node.addEventListener(`${name}start` as any, this.onTransitionStart);
      node.addEventListener(`${name}end` as any, this.onTransitionEnd);
    });
  };
  ...
  resetEffect(node: HTMLElement) {
    ...
    ['transition', 'animation'].forEach(name => {
      node.removeEventListener(`${name}start` as any, this.onTransitionStart);
      node.removeEventListener(`${name}end` as any, this.onTransitionEnd);
    });
  }
