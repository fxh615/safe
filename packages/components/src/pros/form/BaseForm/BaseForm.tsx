...
import { useUrlSearchParams } from '@/pros/utils/useParams';
...
function BaseFormComponents<T = Record<string, any>>(props: BaseFormProps<T>) {
  ...
  const formRef: any = useRef<ProFormInstance<any>>(inlineForm! || ({} as any));
  ...
  const formatValues: any = useMemo(
    () => ({
      /**
  ...
  const responseForm = useMemo(() => {
    ...
    Object.keys(formatValues).forEach((key: string) => {
      Object.defineProperty(response, key, {
        get: () => {
