...
import type { UseEditableUtilType } from '@/pros/utils';
import { arrayMoveImmutable } from '@/pros/utils';
import type { TablePaginationConfig } from '@/table';
import type { SortOrder } from '@/table/interface';
...
export function sortData<T>({ oldIndex, newIndex }: SortDataParams, data: T[]): T[] | null {
  if (oldIndex !== newIndex) {
    const newData = arrayMoveImmutable([...(data || [])], oldIndex, newIndex).filter((el: any) => !!el);
    return [...newData];
  }
