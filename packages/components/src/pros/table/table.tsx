...
import ProCard from '@/pros/card';
import ProForm from '@/pros/form';
import type { ParamsType } from '@/pros/provider';
import { ConfigProviderWrap } from '@/pros/provider';
...
import {genColumnKey, isBordered, parseDefaultColumnConfig, mergePagination, useActionType} from './utils';
...
function TableRender<T extends Record<string, any>, U, ValueType>(
  ...
  const baseTableDom = <Table {...getTableProps() as any} rowKey={rowKey} />;
...
const ProTable = <T extends Record<string, any>, U extends ParamsType, ValueType>(
  ...
  useEffect(() => {
    selectedRowsRef.current = selectedRowKeys!?.map(
      (key: any): T => preserveRecordsRef.current?.get(key) as T,
    );
  }, [selectedRowKeys]);
...
>(
  ...
  const ErrorComponent =
    ...
    const Provider = Container.Provider as any;
  return (
    <Provider initialState={props}>
      <ConfigProviderWrap>
        <ErrorComponent>
